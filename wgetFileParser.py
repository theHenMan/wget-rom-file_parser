...
#GUI file open and file save box
root = tk.Tk()
root.withdraw()
...
file_path_open = filedialog.askopenfilename(title = 'Select the LOG FILE to parse', \
    filetypes = (('text file', '*.txt'), ('log file', '*.log'), ('all files', '*.*')))

try:
    with open(file_path_open, "r") as wget: # Log file generated by wget
...
try:
    file_path_save = filedialog.asksaveasfile(mode = 'w', defaultextension = '.txt', title = 'Save File As...', \
        filetypes = (('text files', '*.txt'), ('all files', '*.*')))
    with open(file_path_save.name, "w") as f:
        ...
        for k, v in name_and_length.items():
            f.write(str(k) + '{{0:>{}}}'.format((rom_filename_length+size_of_rom_length)-len(k)).format(str(v)) + '\n')
    ...
    print("\nCan't save as blank file.\n\nExiting...")

#Open file that was created?
open_file = input("\nDo you want to open the file? (y/n) ")
if open_file.lower().startswith("y"):
    os.startfile(file_path_save.name)